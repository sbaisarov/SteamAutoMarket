...
namespace System.Windows.Forms
{
	...
	public abstract class TabStyleProvider : Component
	{
		...
		protected virtual Brush GetTabBackgroundBrush(int index){
			...
			Color dark = Color.FromArgb(254, 218, 106);
			Color light = Color.FromArgb(254, 218, 106);
			
			if (this._TabControl.SelectedIndex == index) {
                dark = FormComponents.SIMPLE_BACK_COLOR;

                light = FormComponents.SIMPLE_BACK_COLOR;
			...
			if (this._FocusTrack && this._TabControl.Focused && index == this._TabControl.SelectedIndex) {
				...
				switch (this._TabControl.Alignment) {
					case TabAlignment.Top:
						...
						focusBrush = new LinearGradientBrush(focusRect, FormComponents.SIMPLE_BACK_COLOR, FormComponents.SIMPLE_BACK_COLOR, LinearGradientMode.Vertical);
						break;
					case TabAlignment.Bottom:
					...
					case TabAlignment.Left:
						...
						focusBrush = new LinearGradientBrush(focusRect, FormComponents.SIMPLE_BACK_COLOR, FormComponents.SIMPLE_BACK_COLOR, LinearGradientMode.Horizontal);
						break;
					case TabAlignment.Right:
		...
		public virtual Brush GetPageBackgroundBrush(int index){
			...
			//	Capture the colours dependant on selection state of the tab
			Color light = FormComponents.SIMPLE_BACK_COLOR;
			if (this._TabControl.Alignment == TabAlignment.Top){
				light = FormComponents.SIMPLE_BACK_COLOR;
			}
