namespace Steam.TradeOffer
{
    ...
    public class OfferSession
    {
            ...
            string tradeOfferId)
        {
            ...
            if (!this.Request(SendUrl, data, referer, tradeOfferId, out newTradeOfferId))
            {
                var state = this._webApi.GetOfferState(tradeOfferId);
        ...
        public bool SendTradeOffer(string message, SteamID otherSteamId, TradeStatus status, out string newTradeOfferId)
        {
            var data = new NameValueCollection
                           {
                               { "sessionid", this._sessionId },
            ...
            return this.Request(SendUrl, data, referer, null, out newTradeOfferId);
        }
        ...
        /// <param name="newTradeOfferId">The trade offer Id that will be created if successful</param>
        /// <returns>True if successfully returns a newTradeOfferId, else false</returns>
        public bool SendTradeOfferWithToken(
            string message,
            SteamID otherSteamId,
            TradeStatus status,
            string token,
            ...
            out string newTradeOfferId)
        {
            ...
            if (!string.IsNullOrEmpty(resp))
                try
                {
                    ...
                    this.Error = offerResponse.TradeError;
                    Debug.WriteLine($"Error on decline trade offer - {this.Error}");
                }
                catch (JsonException ex)
                {
                    Debug.WriteLine("Error on decline trade offer", ex);
                }
