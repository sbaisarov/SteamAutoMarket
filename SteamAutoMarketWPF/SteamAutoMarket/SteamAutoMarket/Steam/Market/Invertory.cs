namespace SteamAutoMarket.Steam.Market
{
    ...
    public class Invertory
    {
        ...
        public Dictionary<JInvertoryAsset, JDescription> Get(
            long userId,
            int appId,
            int contextId,
            int count = 500,
            bool useAuth = false)
        {
            ...
            var urlQuery = new Dictionary<string, string> { { "count", count.ToString() } };

            var resp = this._steam.Request(url, Method.GET, Urls.Inventory, urlQuery, useAuth);
            ...
            var dic = respDes.Assets.ToDictionary(
                x => x,
